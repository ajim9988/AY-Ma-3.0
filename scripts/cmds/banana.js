 ...
 en: "{p}nanobanana <prompt> [--count <1-4>]\nExample: {p}nanobanana a cute cat on a rooftop at night\nExample: {p}nanobanana futuristic city --count 4",
  ...
  onStart: async function ({ api, event, args, message }) {
    ...
    if (countIdx !== -1 && args[countIdx + 1]) {
      numImages = Math.min(4Math.max(1, parseInt(args[countIdx + 1]) || 1));
...
ents = images.map((b64) => {
        ...
        const buf = Buffer.from(clean, "base64");
        const stream = new Readable();
        stream.push(th.max(1, parseInt(args[countIdx + 1]) || 1));
      args.splice(countIdx, 2);
    }

    const prompt = args.join(" ").();
    if (!prompt) {
      return message.reply role: 0,
    shortDescription: {
      en: "Nano Banana AI image generator",
    },
    longDescription: {
